  ...
  return (
    <div className="card" style={{
      ...
      <button onClick={() => setOpen(o => !o)} style={{
        width:'100%', display:'flex', flexWrap:'wrap', alignItems:'center', gap:14,
...
function OpenCard({ row }) {
  ...
  return (
    <div className="card" style={{ overflow:'hidden', borderColor: open ? 'var(--border-default)' : 'var(--border-subtle)' }}>
      <button onClick={() => setOpen(o => !o)} style={{
        width:'100%', display:'flex', flexWrap:'wrap', alignItems:'center', gap:12,
        padding:'14px 18px', background:'none', border:'none', cursor:'pointer',
      }}>
        <div style={{ width:6, height:6, borderRadius:'50%', background:topColor, boxShadow:`0 0 6px ${topColor}`, flexShrink:0 }} />
        <div style={{ flex:1, minWidth:0, display:'flex', gap:5, flexWrap:'wrap', textAlign:'left' }}>
          {parts.map(p => <span key={p} className={`badge ${partColor(p)}`} style={{ fontSize:9 }}>{p.replace(/_/g,' ')}</span>)}
        </div>
        <span style={{ fontSize:10, color:'var(--text-muted)', fontFamily:'var(--font-mono)', background:'rgba(255,255,255,0.03)', padding:'2px 8px', borderRadius:100, border:'1px solid var(--border-subtle)', flexShrink:0, whiteSpace:'nowrap' }}>
          {row.total_count}d
        </span>
      ...
      {open && (
        <div className="fade-in" style={{ padding:'0 18px 16px', borderTop:'1px solid var(--border-subtle)' }}>
          ...
          <div className="edge-open-grid" style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(180px, 1fr))', gap:8 }}>
          ...
          {filteredGap.map(row => {
            ...
            return (
              <div key={`${row.gap_direction}_${row.gap_bucket}`} className="card" style={{ padding:'18px 20px' }}>
                ...
                <div className="edge-gap-grid" style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(240px, 1fr))', gap:16 }}>
